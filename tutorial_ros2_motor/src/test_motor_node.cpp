...
//kalman parameter
float encoder1_kf_estimate = 0.0f;
float encoder2_kf_estimate = 0.0f;
float encoder1_kf_error = 1.0f;
float encoder2_kf_error = 1.0f;
float kf_process_noise = 0.05f;//
float kf_measurement_noise = 0.2f;//

// Flag to enable or disable Kalman filter
...
float ApplyKalmanFilter(float current_estimate, float *error, float measurement) {
    // Kalman gain
    float kalman_gain = *error / (*error + kf_measurement_noise);
    
    // Update estimate
    current_estimate = current_estimate + kalman_gain * (measurement - current_estimate);
    
    // Update error
    *error = (1 - kalman_gain) * (*error) + kf_process_noise;
...
void Interrupt1A(int pi, unsigned user_gpio, unsigned level, uint32_t tick)
{
    ...
    // 엔코더 방향에 따른 측정값 계산
    int raw_increment = gpio_read(pinum, motor1_dir) ? -1 : 1;
    float measured_value = (float)raw_increment * encoder_resolution;  // 실제 측정값 계산
    
    if (use_kalman_filter) {
        // Kalman 필터 적용
        encoder1_kf_estimate = ApplyKalmanFilter(encoder1_kf_estimate, &encoder1_kf_error, measured_value);
        encoder_count_1A = (float)encoder1_kf_estimate;
    } else {
        encoder_count_1A += raw_increment;
    }
    speed_count_1++;
}
...
void Interrupt1B(int pi, unsigned user_gpio, unsigned level, uint32_t tick)
{
    ...
    if (use_kalman_filter) {
        // Kalman 필터 적용
        encoder1_kf_estimate = ApplyKalmanFilter(encoder1_kf_estimate, &encoder1_kf_error, measured_value);
        encoder_count_1B = (float)encoder1_kf_estimate;
    } else {
        encoder_count_1B += raw_increment;
    }
    speed_count_1++;
}
...
void Interrupt2A(int pi, unsigned user_gpio, unsigned level, uint32_t tick)
{
    ...
    // 엔코더 방향에 따른 측정값 계산
    int raw_increment = gpio_read(pinum, motor2_dir) ? -1 : 1;
    float measured_value = (float)raw_increment * encoder_resolution;  // 실제 측정값 계산
    
    if (use_kalman_filter) {
        // Kalman 필터 적용
        encoder2_kf_estimate = ApplyKalmanFilter(encoder2_kf_estimate, &encoder2_kf_error, measured_value);
        encoder_count_2A = (float)encoder2_kf_estimate;
    } else {
        encoder_count_2A += raw_increment;
    }
    speed_count2++;
}
...
void Interrupt2B(int pi, unsigned user_gpio, unsigned level, uint32_t tick)
{
    ...
    if (use_kalman_filter) {
        // Kalman 필터 적용
        encoder2_kf_estimate = ApplyKalmanFilter(encoder2_kf_estimate, &encoder2_kf_error, measured_value);
        encoder_count_2B = (float)encoder2_kf_estimate;
    } else {
        encoder_count_2B += raw_increment;
    }
    speed_count2++;
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////
