...
//  if (accError > maxAccError)
//    accError = maxAccError;
//  if (accError < -maxAccError)
//    accError = -maxAccError;
...
void Calculate_Odom()
{
  ...
  // Low Pass Filter 적용
  //delta_linear = lambda * delta_distance + (1 - lambda) * prev_lpf_value_;
  //delta_angular = lambda * delta_theta + (1 - lambda) * prev_lpf_value_;
  //prev_lpf_value_ = delta_linear;
  ...
  // 헤딩 값을 -π ~ π로 제한
  //heading_ = fmod(heading_ + M_PI, 2 * M_PI) - M_PI;
}
...
void InfoMotors()
{
  ...
  printf("delta_linear : %10.2f    ||  delta_angular : %10.2f\n", delta_linear, delta_angular);
  printf("ACC  :%11.0d\n", acceleration);
...
public:
  MotorDrive();
  double target_pwm1 = 50.0;
  double target_pwm2 = 50.0;
...
private:
  // 각 게인을 업데이트하는 콜백 함수
  void updatePGain(const std_msgs::msg::Float64::SharedPtr msg);
  void updateIGain(const std_msgs::msg::Float64::SharedPtr msg);
  void updateDGain(const std_msgs::msg::Float64::SharedPtr msg);
  // Subscriber
  rclcpp::Subscription<std_msgs::msg::Float64>::SharedPtr p_gain_sub_;
  rclcpp::Subscription<std_msgs::msg::Float64>::SharedPtr i_gain_sub_;
  rclcpp::Subscription<std_msgs::msg::Float64>::SharedPtr d_gain_sub_;
...
MotorDrive::MotorDrive() : Node("motor_control_node")
{
  ...
  sub_cmd_vel_ = this->create_subscription<geometry_msgs::msg::Twist>("cmd_vel", 10, std::bind(&MotorDrive::cmdVelCallback, this, std::placeholders::_1));
  ...
  subscription_ = this->create_subscription<std_msgs::msg::Int64MultiArray>("/tutorial/teleop", 10, std::bind(&MotorDrive::TeleopCallback, this, _1));
  
  
  
  
  p_gain_sub_ = this->create_subscription<std_msgs::msg::Float64>(
      "/pid/p_gain2", 10, std::bind(&MotorDrive::updatePGain, this, std::placeholders::_1));
  i_gain_sub_ = this->create_subscription<std_msgs::msg::Float64>(
      "/pid/i_gain2", 10, std::bind(&MotorDrive::updateIGain, this, std::placeholders::_1));
  d_gain_sub_ = this->create_subscription<std_msgs::msg::Float64>(
      "/pid/d_gain2", 10, std::bind(&MotorDrive::updateDGain, this, std::placeholders::_1));
}
...
void MotorDrive::updatePGain(const std_msgs::msg::Float64::SharedPtr msg) {
    p_gain2 = msg->data;
    RCLCPP_INFO(this->get_logger(), "Updated p_gain1: %f", p_gain2);
}

void MotorDrive::updateIGain(const std_msgs::msg::Float64::SharedPtr msg) {
    i_gain2 = msg->data;
    RCLCPP_INFO(this->get_logger(), "Updated i_gain1: %f", i_gain2);
}

void MotorDrive::updateDGain(const std_msgs::msg::Float64::SharedPtr msg) {
    d_gain2 = msg->data;
    RCLCPP_INFO(this->get_logger(), "Updated d_gain1: %f", d_gain2);
}
...
void MotorDrive::TeleopCallback(const std_msgs::msg::Int64MultiArray::SharedPtr msg)
{
