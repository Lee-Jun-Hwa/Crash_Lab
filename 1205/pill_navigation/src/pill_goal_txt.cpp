...
public:
    GoalNavigator() : Node("goal_navigator")
    {
        ...
        if (!readGoalFromFile("/home/crashlab_rpi/robot_ws/src/pill_navigation/config/destination.txt", target_pose_))
        {
...
private:
    ...
    {
        std::ifstream file("/home/crashlab_rpi/robot_ws/src/pill_navigation/config/destination.txt");
        if (!file.is_open())
        {
